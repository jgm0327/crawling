...
public class ArtandStudyCrawling {
	...
	public List<Lecture> getSaleLecture() throws IOException {
		...
		for(int i = 1; i<=4;i++){
			...
			System.out.println("categorylen = " + categorylen);

			for (int j = 1; j <= categorylen; j++){
				document = Jsoup.connect("https://www.artnstudy.com/n_lecture/LecList.asp?LessonPart=" + i + "0" + j).get();

				System.out.println(sub_category[i][j]);
	...
	private int getSalePrice(Element content) {
		...
		if (elements.isEmpty()) {
			...
			String salePrice = elements.text();
	...
	private int getPrice(Element content) {
		...
		priceInt = Integer.parseInt(numericPart);
